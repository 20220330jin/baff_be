...
//        String redirectUrl = "https://onlymefe.vercel.app/user/oauth-response/" + token + "/" + EXPIRATION_TIME_SECONDS;
//        response.sendRedirect(redirectUrl);


        // --- 로컬 환경에서 사용시 아래 코드로 교체 ---
        createAndSetCookieForLocal(response, "accessToken", token);
        String localRedirectUrl = "http://localhost:3000/user/oauth-response/" + token + "/" + EXPIRATION_TIME_SECONDS;
    ...
    private void createAndSetCookieForProduction(HttpServletResponse response, String key, String value) {
        ...
        String cookieValue = String.format(
                "%s=%s; Path=/; Max-Age=%d; SameSite=None; Secure; HttpOnly=true; Domain=onlymebe.onrender.com",
